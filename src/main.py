...
def upload_dataset(dataset_id: UUID, dataset: UploadFile = File(...), dataset_service: DatasetService = Depends()):
...
def read_item(dataset_id: str, version_id: UUID, dataset_service: DatasetService = Depends()):
...
def train_model(dataset_id: str, version_id: UUID, ml_service: MlService = Depends()):
